...
<template>
  <div class="bg-[#1E1E20] rounded-2xl mb-4 shadow-lg w-[calc(100vw-2rem)] min-h-[390px] md:w-auto sm:w-auto">
    <header class="flex flex-col item-center rounded-t-2xl p-3.5">
      <img src="/icons/left-arrow.svg" alt="Back arrow" class="w-5 h-4 cursor-pointer" @click="$emit('back')" />
      <CloseButton />
      <div class="flex items-center mt-4 gap-2">
        <span
          class="flex items-center justify-center bg-gradient-to-r from-deskree-500 to-deskree-600 rounded-full w-8 h-8">
          <img :src="props.feedback.icon" :alt="props.feedback.title" class="w-4 h-5" />
        </span>
    ...
    <div class="flex px-4 gap-2 w-full flex-col">
      <form @submit.prevent="sendFeedback" class="w-full">
        ...
        <div class="flex flex-col gap-4 mt-2">
          <span v-if="image === undefined"
            class="text-xs text-deskree-300 hover:text-white cursor-pointer transition-colors"
            @click="handleTakeScreenshot">Take a screenshot</span>
          <button v-else type="button" @click="image = undefined"
            ...
            style="background-position: bottom right; background-size: 180px" :disabled="loading">
